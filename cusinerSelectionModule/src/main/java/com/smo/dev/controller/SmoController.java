...
public class SmoController {
	...
	public ResponseEntity<List<SmoBaseDto>> getAllCheffList(SmoBaseDto smoBaseDto) {
		...
		List<SmoBaseDto> smoDtoList = smoService.getCheffListByLocation(location,
				smoBaseDto.getBookingStartTime(),smoBaseDto.getBookingEndTime());
		if(!smoDtoList.isEmpty()) {
			return new ResponseEntity<List<SmoBaseDto>>(smoDtoList,HttpStatus.FOUND);
		}
			return new ResponseEntity<List<SmoBaseDto>>(smoDtoList,HttpStatus.NOT_FOUND);
	}
